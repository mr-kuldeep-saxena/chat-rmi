...
/**
 * This class is an implementation of client side message receiver
 * used to handle messages from the remote server to the client 
...
public class MessageReceiverImpl implements MessageReceiver, Serializable 
{
	...
	public boolean receiveMessage(Message message) throws RemoteException 
	{
		...
		ChatWindow cw = Utilities.getDefaultInstance().getOpenedWindow(message.getFromUser());
		if (cw == null)
		{
			cw = new ChatWindow(message.getFromUser(), null);
			Utilities.getDefaultInstance().addOpenedWindow(message.getFromUser(),cw);
		}
	 ...
	 * @param userID User id of person for whom status is received
	 * @param status new status of the user
	 */
	public void updateUserStatus(String userID, int status) throws RemoteException 
	{
		...
		if (status == UserStatus.USER_ONLINE)
			PopupMessageHandler.getDefaultInstance().addNewMessage(userID +" is now Online");
		else if (status == UserStatus.USER_OFFLINE)
			PopupMessageHandler.getDefaultInstance().addNewMessage(userID +" is now Offline");
		
		for (int i=0; i < groups.length; i++)
		{
			UserSettings us []= Utilities.getDefaultInstance().getUsersInMap(Utilities.getDefaultInstance().getUsersInGroup(groups[i]));
	...
	{
		HashMap <String,UserSettings> users = Utilities.getDefaultInstance().getUsersInGroup("all");
		users.put(newUser.getUserID(), newUser);
		Utilities.getDefaultInstance().updateUserGroupData("all", users);
		Login.getDefaultInstance().updateGroupData();
		PopupMessageHandler.getDefaultInstance().addNewMessage(newUser.getUserID()+" is added to the server");
	}
	 ...
	 *  @return ip address of the client to whom request is coming or null when user does not accespts the file
	 */
	public String acceptFile (String fileName, String fromUser) throws RemoteException
	{
		try
		{
			int returnValue = JOptionPane.showConfirmDialog(null, "User " + fromUser + " is sending you file " + fileName +" accept?", "File", JOptionPane.YES_NO_OPTION );
	...
	public void run ()
	{
		while (true){try{wait();}catch (Exception e){}}		
	}
	
	/**
	 * updates user status
	 * @param ofUser of which user status been updated
	 * @param message new message
	 * @param online is online status message changed or offline
	 * @return status of operation
	 * @throws RemoteException in case of error
	 */
	public int updateNewStatusMessage (String ofUser, String message, boolean online) throws RemoteException
	{
		String groups [] = Utilities.getDefaultInstance().getUserGroups();
		for (int i=0; i < groups.length; i++)
		{
		...
		Login.getDefaultInstance().repaintGroupData();
		//Login.getDefaultInstance().updateNewStatusMsg (ofUser, message, online);
