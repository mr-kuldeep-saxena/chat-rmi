...
/**
 * This class handles all the communication from the client to the server
 * It maintains the connection to the server and calls remote method on behalf of client application
...
public class ClientSideCommunicator 
{
	...
	/**
	 * block gets executed when class is loaded
	 * loads ip and port info and start connecting to server
	 */
	static 
	{
	...
	private static void connectToServer ()
	{
		try
		{
			cs = (ChatServer) Naming.lookup("rmi://"+ip +":" + port + "/"+ OBJECT);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	 ...
	 * @param userData userData object to be stored at server
	 */
	public void registerUser (UserData userData)
	{
		try
		{	
			JOptionPane.showMessageDialog (null, Utilities.getDefaultInstance().mapMessageCode(cs.signUpUser(userData, true)));
		}
		catch (Exception e)
		{
			e.printStackTrace();
	 ...
	 * @param mr client remote object
	 * @return user data after validation, or null, or data with status only
	 * 1) user data with values when user validated
	 * 2) user data null when user not found
	 * 3) user data with status when some error in validating like invalid password 
	 */
	 ...
	 * @param userID user id for whom data is to be returned
	 ...
	 * @param newUserID new user to be added to given group
	 * @param groupName group name
	 * @param isNewGroup is the group is new or existing
	 * @return status of operation
	 */
	public int addUserToGroup (String fromUser, String newUserID, String groupName, boolean isNewGroup)
	{
		try
		{
			return cs.addUserToGroup(fromUser, newUserID, groupName, isNewGroup);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	 ...
	 * @param user for whom message needs to be deleted
	 * @return status of the operation
	 */
	...
	/**
	 * calls remote signupuser data with false as second parameter which tells that its a update to the user data at server
	...
	public int updateUserData (UserData ud)
	{
		try
		{
			...
			JOptionPane.showMessageDialog (null, Utilities.getDefaultInstance().mapMessageCode(status));
			return status;
	 ...
	 * @param user for which offline message needs to be retrieved
	...
	public int sendFileAcceptanceToUser (String toUser, String fromUser, String fileName) 
	{
		try
		{
