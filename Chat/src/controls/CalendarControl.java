		...
		implements ActionListener, ChangeListener
{
	...
	final private String STR_months [] =
	{
		"January",
		"Feburary",
		"March",
	...
	private void setComponentsProperties ()
	{
		...
		L_month.setFont (new java.awt.Font ("regular", Font.PLAIN | Font.ITALIC, 9));
		L_year.setFont (new java.awt.Font ("regular", Font.PLAIN | Font.ITALIC, 9));
		setBorder (new CompoundBorder (new BevelBorder(BevelBorder.RAISED), new CompoundBorder(new BevelBorder(BevelBorder.LOWERED), new EmptyBorder(1,1,1,1))));
		for ( int i = 0; i < B_days.length; i++)
		{
	 ...
	 * @param noOfDays noofdays in the month
	 */
	private void createCalendar (int noOfDays)
	{
		...
		for ( i = startIndex; i < (noOfDays +startIndex); i++)
		{
			...
			if (! bool_yearFirst)
			{
				B_days [i].setActionCommand (appendLeadingZero(currentDay) + "/" + appendLeadingZero(int_month+1)+ "/" + int_year);
			}
			else
			{
				B_days [i].setActionCommand (int_year + "/" + appendLeadingZero(int_month+1)+ "/" + appendLeadingZero(currentDay));
				//System.out.println(int_year + "/" + appendLeadingZero(int_month+1) + "/" + appendLeadingZero(currentDay));
			}
		...
		if (!bool_yearFirst)
			this.TF_date.setText(appendLeadingZero(int_day) + "/" + appendLeadingZero(int_month+1) + "/" + int_year);
		else
			this.TF_date.setText(int_year + "/" + appendLeadingZero(int_month+1) + "/" + appendLeadingZero(int_day));
	 ...
	 * @param int_month month in which days has to be calculated 
	 * @param int_year year of the given month 
