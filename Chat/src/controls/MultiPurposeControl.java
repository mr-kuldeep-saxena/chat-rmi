...
/**
 * This class a multipurpose control which has various drawing options
 * 1) Gradient to draw the text gradient
 * 2) Wave to draw the text using waves
 * 3) Comic to draw  the text using comic 
 * 4) FADED to draw the text using faded
 * 5) ROSE to draw the text using rose image
 * Also it has two other options which can be applied altogether to the above options
 * 1) 3D text draw draws text in 3D format
 * 2) 3D border draw 3D border around text 
...
public class MultiPurposeControl extends JPanel
{
	 ...
	 * calls other constructor with empty string 
	 *
	 */
	 ...
	 * @param STR_text text to be drawn
	 */
	...
	public void paint (Graphics g)
	{
		...
		if ((STR_text.length()) % 2 == 1 )
			charsPerLine ++;
		//System.out.println(charsPerLine + " " + STR_text.length() + " " + rows);
		...
		if (currentValue == FLOWER || currentValue == FADED || currentValue == GRADIENT)
		{
			setPaintMode (g2);
			drawDefault(g2, 4);
			return;
	...
	/**
	 * draws the text to the gievn x position
	 * and if chars exceeds the width it wraps the text to next line
	 * @param g2 graphics object used for drawing
	 * @param x x point to be draw the text
	 */
	private void drawDefault (Graphics2D g2, int x)
	{
		...
		for (int i=0; i < rows; i++)
		{
			...
			{
				toDrawn = STR_text.substring(startIndex, startIndex + charsPerLine);
			}
			else
			{
			...
			if (strikeThrough)
			{
				...
				//g2.drawLine(x, (fm.getHeight () * i) +y, x + fm.stringWidth(toDrawn), (fm.getHeight () * i) +y);
			}
			y += fm.getHeight();
			if (underLine)
			{
				g2.drawLine(x, fm.getHeight() * (i+1), x + fm.stringWidth(toDrawn), fm.getHeight() * (i+1));
			}
	 ...
	 * @param g2 graphics object used for drawing text
	 */
	 ...
	 * @param g2 graphics object used to draw text
	 */
	 ...
	 * @param g2 graphics object which is been updated
	 */
	private void setPaintMode (Graphics2D g2)
	{
		...
		if (currentValue == GRADIENT)
		{
			...
			if (foreColor.getRed() < 30 &&  foreColor.getBlue()  < 30 && foreColor.getGreen() < 30 )
			{
				secondColor = Color.yellow.darker();//foreColor.brighter().brighter().brighter().brighter();
			}
			GradientPaint gp=new GradientPaint(0, 0, secondColor , getWidth()/10, getHeight()/10, foreColor, true);
			g2.setPaint (gp);
			return;
		}
		// if faded value is selected creating gradient paint with fading at boundary 
		if (currentValue == FADED)
		{
			GradientPaint gp=new GradientPaint(0, 0, foreColor, getWidth()- 5, getHeight(), Color.white.darker(), true);
			g2.setPaint (gp);
			return;
		}
		...
		if (currentValue == FLOWER)
		{
			...
			Rectangle2D rect = new Rectangle2D.Double(0,0,fm.stringWidth("W"),getHeight());
	...
	/**
	 * @param threeDBox The threeDBox to set.
	 */
	 ...
	 * @param g2 graphics object used to draw the text
	 */
	...
	/**
	 * calculates the preffred size according to the container size and the text size
	 * @return
	 */
	private Dimension calculatePreferredSize ()
	{
		Dimension d = null;
		if (containerSize == null || containerSize.width < 1|| containerSize.height <1)
		{
