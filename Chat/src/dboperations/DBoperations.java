...
/**
 * This class is used perform database related operations. currently it is using xml file format
 * but this layer can be changed without changing other part of the application.
 * it also contains several list to maintain highly accessible data in ram.
...
public class DBoperations 
{
	...
	/**
	 * signed in user list which is used by the user status notifier to update other users
	 */
	private ArrayList<String> signInUsers = new ArrayList<String>(); // new users list
	/**
	 * newly registered user list which is used by the user status notifier to update other users
	 */
	private ArrayList<String> newUsers = new ArrayList<String>(); // new users list
	/**
	 * signed out user list which is used by the user status notifier to update other users
	 */
	private ArrayList<String> signOutUsers= new ArrayList<String>(); // sign out user list
	/**
	 * new messages holder
	 */
	...
	public int storeUserSettings (UserData userData)
	{
		try
		{
			...
			// user object is stored in an xml file with the convention userid.xml
			FileOutputStream out = new FileOutputStream ("userDB/" + userData.getUserID() +".xml");
			...
			out.write(("</USER>\n").getBytes());
			out.close();
			logger.writeLog("User data for " + userData.getUserID() + " save to DB successfully");
		}
		catch (Exception e)
		{
			e.printStackTrace();
			logger.writeLog("Failed to save user data for " + userData.getUserID() + " to the DB\n" + e.getMessage());
			return OperationStatus.OPERATION_FAILURE; 
		}
		
		UserSettings us = new UserSettings (userData.getUserID(), userData.getUserName(), false, UserStatus.USER_ONLINE);
	 ...
	 * @param userID user id for which setting needs to be loaded
			...
			for (int i=0; i <childs.getLength(); i++)
			{
				...
				if (childs.item(i).getNodeName().equalsIgnoreCase("GROUPS"))
				{
					...
					for (int k=0; k < groups.getLength(); k++)
					{
						...
						Node node = groups.item(k).getAttributes().getNamedItem("name");
						...
						{
							StringTokenizer st = new StringTokenizer (users.item(j).getTextContent(),",");
							uid = new UserSettings [st.countTokens()];
							int l =0;
							while (st.hasMoreTokens())
							{
								String id=st.nextToken();
								uid [l] = new UserSettings(id, getNameForID(id), false, isUserOnline(id) == true ? UserStatus.USER_ONLINE:UserStatus.USER_OFFLINE);
								if (getAllUsers().get(id) != null)
								{
									System.out.println(id + " " + getAllUsers().get(id).getUserOnlineStatusMsg());
									uid [l].setUserOnlineStatusMsg(getAllUsers().get(id).getUserOnlineStatusMsg());
									uid [l].setUserOfflineStatusMsg(getAllUsers().get(id).getUserOfflineStatusMsg());
								}
								l++;
							}
						}
	...
	/**
	 * check whether user is already registered, check whether the file with the name already exists or no 
	...
	public HashMap <String,UserSettings> loadAllUserList ()
	{
		...
		// looks in the userDB directory and searches for all the files with .xml extension 
		if (!F_dir.exists())
			F_dir.mkdir();
		if (F_dir.isDirectory())
		{
			...
			for (int i=0; i < files.length; i++)
			{
				if (!files[i].getName().endsWith(".xml"))
					continue;
				UserData ud = loadUserSettings(files[i].getName().substring(0, files[i].getName().indexOf(".xml")));
				...
				UserSettings us = new UserSettings (ud.getUserID(), ud.getUserName(), false, UserStatus.UNKNOWN);
	...
	/**
	 * this function adds given user to the given group
	 * first it reads data from db then it writes back the updated data 
	 * @param fromUser the user id from where request is coming
	 * @param userID user to be added to the group
	 * @param groupName group in which user is going to be added
	 * @param isNewGroup to distinguish between new and old group 
	 * @return status of the operation
	 */
	public int addUserToGroup (String fromUser, String userID, String groupName, boolean isNewGroup)
	{
		...
		else
		{
			...
			{
				if (userGroups.get(i).getGroupName().equalsIgnoreCase(groupName))
				{
			...
			usNew [i] = new UserSettings (userID,userID, false,UserStatus.UNKNOWN);
	 ...
	 * @param user for which messages has to be deleted
	 * @return status of the operation
	 */
	public int deleteOfflineMessages (String user)
	{
		...
		{
			logger.writeLog("Error deleting offline messages: " + e.getMessage());
			return OperationStatus.OPERATION_FAILURE;
		}
	}
	 ...
	 * @param user for which messages to be loaded
	 * @return messages
	 */
	 ...
	 * @param user for which message to be written
	 * @param message messages to be written 
	 * @return status of the operation
	 */
	public int writeOffLineMessage (String user, Message message)
	{
		...
		try
		{
			...
			File f = new File("offline_messages/" + user + "_off.msg");
			// first reads offline messages if any then adds new message to it and writes it back
			if (f.exists())
			{
				in = new ObjectInputStream (new FileInputStream(f));
