 ...
 *
 */
public class ArchiveViewer extends JFrame implements ComponentListener, TreeSelectionListener, ListSelectionListener, ActionListener
{
			...
			if ( f != null)
			{
				...
				for (int i=0; i < dirs.length; i++)
				{
					if (dirs[i].isDirectory())
					{
						try
						{
							String with = dirs[i].getName().substring(dirs[i].getName().indexOf("$")+1);
				...
				((DefaultTreeModel)TR_users.getModel()).setRoot(populateMessages(directories));
			}
	...
	private void initComponents ()
	{
		...
		((DefaultTreeModel)TR_users.getModel()).setRoot(populateMessages(null));
		...
		TR_users.getSelectionModel().setSelectionMode(TreeSelectionModel.SINGLE_TREE_SELECTION);
	...
	public void componentResized(ComponentEvent e) 
	{
		...
		SLP_pane.updateUI();
		P_rightTop.setPreferredSize(new Dimension(P_right.getPreferredSize().width, 100));
	...
	private DefaultMutableTreeNode populateMessages(Vector <String> messages)
	{
		...
		root = new DefaultMutableTreeNode (new UserSettings("Messages","Messages",true,UserStatus.UNKNOWN));
			...
			for (int i=0; i < messages.size(); i++)
			{
				if (users!= null)
				{
					if (users.containsKey(messages.get(i)))
							root.add(new DefaultMutableTreeNode(users.get(messages.get(i))));
					else
						root.add(new DefaultMutableTreeNode(new UserSettings(messages.get(i), messages.get(i), false, UserStatus.UNKNOWN)));
				}
				else
					root.add(new DefaultMutableTreeNode(new UserSettings(messages.get(i), messages.get(i), false, UserStatus.UNKNOWN)));
			}
	...
	/**
	 * @param msgDir The msgDir to set.
	 */
	...
	public void valueChanged(TreeSelectionEvent e) 
	{
		if (e.getSource() == TR_users)
		{
			...
			tableModel.clear();
			TB_dates.updateUI();
			DefaultMutableTreeNode node = (DefaultMutableTreeNode)TR_users.getLastSelectedPathComponent();
			UserSettings us = (UserSettings) node.getUserObject();
			String selectedUser = Utilities.getDefaultInstance().getCurrentUser() + "$" + us.getUserID();
	...
	public void valueChanged(ListSelectionEvent e) 
	{
		...
		TP_messages.setText("");
		ArrayList <Message> messages = Utilities.getDefaultInstance().getArchivedMessages (f.toString());
		if (messages!= null)
			for (int i=0; i < messages.size (); i++)
			{
				Message m = messages.get(i);
				m.setShowTimeStamp(true);
				Utilities.getDefaultInstance().showMessage(messages.get(i), this,TP_messages,displayDoc,displayStyle, messages.get(i).getType() == MessageType.LOCAL ? false : true);
			}
