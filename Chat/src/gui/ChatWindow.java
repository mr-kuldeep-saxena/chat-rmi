...
/**
 * This class provides user a chatting window to the user
 * the created window object remains same for a given user, any other request to open the window
 * for the same user is handled before creating this window object
 * from this window multiple users can be selected if user is wishing to send same message to multiple users
 * also the default user selected can not be unselected
 * @author Kuldeep Saxena
 *
 */

public class ChatWindow extends JFrame implements ActionListener, ChangeListener, KeyListener, WindowListener
{
	/**
	 * @serial serial key
	 */
	...
	private JButton B_bold, B_italic, B_underLine, B_strikeThrough, B_showInMessage, B_colorPicker, B_fontPicker;
	private JPanel P_center, P_centerLeft, P_centerRight, P_south, P_southSub1, P_southSub2;
	...
	private Border upperBorder = null;//new CompoundBorder(new BevelBorder(BevelBorder.RAISED),new BevelBorder(BevelBorder.RAISED));
	private Border loweredBorder = new CompoundBorder(new BevelBorder(BevelBorder.LOWERED),new BevelBorder(BevelBorder.LOWERED));
	...
	/**
	 * the group in which this user belongs
	 * this chat window will contain the list of all the users within this group
	 */
	...
	/**
	 * flag indicates whether archieve operation in progress in that case we don't need to add the message again
	 */
	 ...
	 * Default Contructor
	 * initializes GUI
	 * @param userID userid to whom this window belong
	 * @param groupName defualt group for which data need to be loaded
	 */
	...
	private void initComponents ()
	{
		...
		B_italic = new JButton(new ImageIcon(new ImageIcon("resources/images/italic.gif").getImage().getScaledInstance(iconWidth,iconHeight, Image.SCALE_SMOOTH)));
		B_underLine = new JButton(new ImageIcon(new ImageIcon("resources/images/underline.gif").getImage().getScaledInstance(iconWidth,iconHeight, Image.SCALE_SMOOTH)));
		B_strikeThrough = new JButton(new ImageIcon(new ImageIcon("resources/images/strikethrough.gif").getImage().getScaledInstance(iconWidth,iconHeight, Image.SCALE_SMOOTH)));
		B_colorPicker = new JButton(new ImageIcon(new ImageIcon("resources/images/colorpicker.gif").getImage().getScaledInstance(iconWidth,iconHeight, Image.SCALE_SMOOTH)));
		B_fontPicker = new JButton(new ImageIcon(new ImageIcon("resources/images/fontpicker.gif").getImage().getScaledInstance(iconWidth,iconHeight, Image.SCALE_SMOOTH)));
		B_showInMessage = new JButton(new ImageIcon("resources/images/messagebox.gif"));
		...
		messageBoxDoc.setParagraphAttributes(0, messageBoxDoc.getLength(), messageBoxStyle, true);
		...
		B_showInMessage.setToolTipText("Show your message to the recepient in message box");
		...
		CBMI_showTimestamp.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F2, KeyEvent.KEY_LOCATION_UNKNOWN));
		MI_showRecent.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F3, KeyEvent.KEY_LOCATION_UNKNOWN));
		MI_showFullArchive.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F4, KeyEvent.KEY_LOCATION_UNKNOWN));
		
		/**
		 * listener to handle list selection event on selection user from the list
		 */
		LT_users.getSelectionModel().addListSelectionListener(new ListSelectionListener()
		{
			boolean handle = true;
			public void valueChanged(ListSelectionEvent selected) 
			{
				...
				{
					UserSettings cb;
					cb = ((UserSettings)LT_users.getModel().getElementAt(LT_users.getSelectedIndex()));
					if (((CheckBoxModel)LT_users.getModel()).getDefaultUserID()!= null) 
						if (((CheckBoxModel)LT_users.getModel()).getDefaultUserID().equalsIgnoreCase(cb.getUserID()) )
							return;
					if (Utilities.getDefaultInstance().getOpenedWindow(cb.getUserID()) != null && ! cb.isSelected())
					{
						return;
					}
					...
					if (cb.isSelected())
					{
						...
						if (Utilities.getDefaultInstance().getOpenedWindow(userID) == null)
							Utilities.getDefaultInstance().addOpenedWindow(userID, ChatWindow.this);
					}
					else
					{
						Utilities.getDefaultInstance().removeWindowForUser (cb.getUserID());
					}
					handle = false;
					//LT_users.updateUI();
	 ...
	 * listens for various component events
	 */
	public void actionPerformed (ActionEvent e)
	{
		...
		if (e.getSource() == MI_sendFile)
		{
			...
			if (fileName == null)
				return;
			ClientSideCommunicator.getDefaultInstance().sendFileAcceptanceToUser(getSelectedUsers().get(0), Utilities.getDefaultInstance().getCurrentUser(),fileName);
/*			if (inetAddress.equalsIgnoreCase("USER IS OFFLINE"))
			{
				JOptionPane.showMessageDialog(null, "User is offline, can't send file");
				return;
			}
			if (inetAddress == null)
			{
				JOptionPane.showMessageDialog(null, "user cancelled request");
				return;
			}
*//*			try
			{
				Socket s = new Socket (inetAddress, 9200);
		 ...
		 {
				if (MI_showFullArchive.getText().trim().equalsIgnoreCase("Show Full Archive"))
				{
		...
		{
			// sets variable and calls the updateMessages which inserts message in the chat window
		...
		if (e.getSource() == B_send)
		{
			...
			{
				JOptionPane.showMessageDialog(ChatWindow.this, "Please selected at least one user to send message");
			}
			...
			try
			{
				textMessage.setTextMessage(messageBoxDoc.getText(0, messageBoxDoc.getLength()));
			}
			catch (Exception e1)
			{
			...
			// sets message other properties, these properties are set later because these are related to displaying message
			// in message box
		...
		{
			colorChooserPanel.setLocation(B_send.getLocationOnScreen().x + 30, B_send.getLocationOnScreen().y + 25);
		...
		if (e.getSource() == B_fontOk)
		{
			...
			selectedFontSize= Integer.parseInt ((String) CMB_fontSizes.getSelectedItem());
			StyleConstants.setFontFamily(messageBoxStyle, selectedFontName);
			StyleConstants.setFontSize(messageBoxStyle, selectedFontSize);
		...
		// applying attribute to the current text component so that effect of change can be visible to the user
		messageBoxDoc.setParagraphAttributes(0, messageBoxDoc.getLength(), messageBoxStyle, true);
	}
	 ...
	 * @param groupName the group for which users need to be loaded
	 * @param userID default selected user id
	 */
	private void populateList (String groupName, String userID)//, boolean all)
	{
		...
		// is null when message is coming from the remote and a new window is opened at the client 
		if (groupName ==null)
		{
			UserSettings us = Utilities.getDefaultInstance().getUserSettings(userID);
		...
		// calls first User in group which return a map containing user name as key and its setting as value, which
		// is then used to return all the value		
		UserSettings userSettings []= Utilities.getDefaultInstance().getUsersInMap(Utilities.getDefaultInstance().getUsersInGroup(groupName));
	...
	/**
	 * this function is called to show message in the display box
	 * called by the local client as well as by the MessageReceiverImpl for remote message
	 * @param message message to be displayed
	 * @param local indicates whether its a local message or not
	 */
	public void showMessage (Message message, boolean local)
	{
		...
		Utilities.getDefaultInstance().showMessage(message, this, TP_display, displayBoxDoc, displayBoxStyle, local);
	...
	/**
	 * Key Typed Event listener
	 * listener for release event for message box 
	 * and checks whether the key is enter and if calls the sendMessage function by creating
	 * a false event 
	 * otherwise it simply enable/disables send button by checking text data in the field 
	 */
	public void keyReleased(KeyEvent e) 
	{
		if (e.getSource() == TP_messageBox)
		{
			...
			{
				if (TP_messageBox.getText()!= null && TP_messageBox.getText().trim().length() > 0)
				{
					actionPerformed(new ActionEvent(B_send, ActionEvent.ACTION_PERFORMED, "Send"));
				}
				TP_messageBox.setText("");
			}
	...
	private void showFontDialog ()
	{
		...
		D_font.setLocation(B_fontPicker.getLocationOnScreen().x, B_fontPicker.getLocationOnScreen().y+20);
	...
	private void initFontDialog()
	{
		...
		CMB_fontNames = new JComboBox (		GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames());
		CMB_fontSizes = new JComboBox (new String[]{"10", "12", "14", "16", "18", "20", "22" , "24", "28", "32", "36", "42", "48", "52"});
	...
	private void initColorDialog ()
	{
		...
		colorChooserPanel.setSize(colorChooser.getPreferredSize().width, colorChooser.getPreferredSize().height + 50);
	...
	/**
	 * @param dimSelected The dimSelected to set.
	 */
	...
	/**
	 * @param noneSelected The noneSelected to set.
	 */
	...
	/**
	 * @param threeDBoxSelected The threeDBoxSelected to set.
	 */
	...
	/**
	 * @param threeDSelected The threeDSelected to set.
	 */
	...
	/**
	 * @param waveSelected The waveSelected to set.
	 */
	...
	/**
	 * @param selectedFontName The selectedFontName to set.
	 */
	...
	/**
	 * @param selectedFontSize The selectedFontSize to set.
	 */
	...
	/**
	 * @param comicSelected The comicSelected to set.
	 */
	...
	/**
	 * @param flowerSelected the flowerSelected to set
	 */
	...
	private void updateMessages ()
	{
		...
		TP_display.setText("");
		redrawing = true;
		File f = new File ("archiveData/" + Utilities.getDefaultInstance().getCurrentUser() +"$" + userID);
		String files [] = null;
		if (f.isDirectory())
			files = f.list();
		else
		...
		if (showFullArchive)
		{
			...
			{
				ArrayList <Message> oldMessages = Utilities.getDefaultInstance().getArchivedMessages (f.getAbsolutePath() +"/"+files[i]);
				if (oldMessages != null)
		...
		else if (showArchive)
		{
			...
			// loads old archived messages 
			ArrayList <Message> oldMessages = Utilities.getDefaultInstance().getArchivedMessages (f.getAbsolutePath() +"/"+files[files.length-1]);
			if (oldMessages != null)
	...
	/**
	 * @param dancingSelected The dancingSelected to set.
	 */
