...
/**
 * This class is the starting point of the application at the client side. it provides a gui to user from where he/she can log
 * in and use the services.
 * @author Kuldeep Saxena
 *
 */

public class Login extends JFrame implements  ActionListener, TreeSelectionListener, MouseListener, WindowListener, ChangeListener, KeyListener
{
	/**
	 * @serial serial key
	 */
	...
	private JMenu M_options, M_view, M_help, M_onlineStatusMsg, M_offlineStatusMsg, M_theme;
	private JMenuItem MI_signout, MI_editprofile, MI_add, MI_close, MI_about, MI_archiveViewer;
	...
	private JRadioButtonMenuItem RBMI_gradient, RBMI_default, RBMI_metal, RBMI_motif;
	...
	private Login ()
	{
		...
		P_newUser.setPreferredSize (new Dimension (getWidth() - 15, getHeight()/6 )) ;
		P_register.setPreferredSize (new Dimension(getWidth(), P_newUser.getPreferredSize().height/3)) ;
		P_enterChat.setPreferredSize (new Dimension(getWidth() -15, getHeight()/2)) ;
		P_options.setBounds ( (getWidth()/2) - (wid/2), 90, wid, getHeight()/6) ;
		...
		TF_offlineStatus.setBorder(new TitledBorder("My Offline Status Message"));
		
		L_chatName.setBounds ( 10, 20, getWidth () / 2 - 30, 23 ) ;
		TF_userID.setBounds ( L_chatName.getX() + L_chatName.getWidth() + 2 , 20, getWidth () / 2 - 10, 23 ) ;
		L_password.setBounds ( 10, 55, getWidth () / 2 - 30, 23 ) ;
		PF_password.setBounds ( L_password.getX() + L_password.getWidth() + 2 , 55, getWidth () / 2 - 10, 23 ) ;
		...
		RBMI_motif.addActionListener (this) ;
		...
		float_windowWidthRatio = ((float)getWidth ()) / ((float)Properties.getScreenWidth());
		float_windowHeightRatio = ((float)getHeight ()) / ((float)Properties.getScreenHeight()) ;
		setResizable(false) ;
		setVisible(true) ;
		...
		try
		{
			...
			if (user != null && pwd != null)
			{
				...
				actionPerformed(new ActionEvent(B_enter, ActionEvent.ACTION_PERFORMED, "Enter"));
	...
	private void updateComponents ()
	{
		...
		SwingUtilities.updateComponentTreeUI(ArchiveViewer.getDefaultInstance());
	}
...
/*
		...
		if (AE_action.getSource() == RBMI_motif)
		{
			try
			{
...
/*				SwingUtilities.updateComponentTreeUI(this);
		...
		if ( AE_action.getSource() == B_enter )
		{
			...
			UserData ud = ClientSideCommunicator.getDefaultInstance().validateUser(TF_userID.getText(), new String(PF_password.getPassword()), (MessageReceiver)MessageReceiverImpl.getDefaultInstance());
			if (ud == null)
			{
				JOptionPane.showMessageDialog(null, Utilities.getDefaultInstance().mapMessageCode(OperationStatus.SERVER_NOT_AVAILABLE));
				return;
			}
			if (ud.getStatus() != OperationStatus.SUCESSFUL_LOGIN)
			{
				// displays any error message
				JOptionPane.showMessageDialog(null,Utilities.getDefaultInstance().mapMessageCode(ud.getStatus()));
				return;
			}
			try
			{
				...
				if (CB_remember.isSelected())
				{
					...
					p.store(new FileOutputStream ("usersettings.config"), "User Settings");
				}
				else
				{
			...
			allUsers = ClientSideCommunicator.getDefaultInstance().loadAllUsers();
			...
			if (ud.getUserOnlineStatusMsg() != null && ud.getUserOnlineStatusMsg().trim().length() > 0)
			{
				onlineMsg = ud.getUserOnlineStatusMsg();
			}
			if (ud.getUserOfflineStatusMsg() != null && ud.getUserOfflineStatusMsg().trim().length() > 0)
			{
			...
			TR_userGroups.getSelectionModel().setSelectionMode(TreeSelectionModel.SINGLE_TREE_SELECTION);
			TR_userGroups.setOpaque(false);
			Utilities.getDefaultInstance().setScrollWindow(P_desktop, true);
			GradientPanel gp = new GradientPanel (new Color(238, 228, 218),new Color(180,155,130));
			getContentPane().add(gp, BorderLayout.CENTER);
			TransparentScrollPane tp;
			gp.setLayout(new BorderLayout());
			gp.add(tp= new TransparentScrollPane(TR_userGroups), BorderLayout.CENTER);
			...
			// after login calls the communicator to load offline message if any
			Message [] messages = ClientSideCommunicator.getDefaultInstance().getOffLineMessages (Utilities.getDefaultInstance().getCurrentUser());
		...
		if (AE_action.getSource() == MI_add)
		{
			...
			if (!currentSelectedUser.isGroup())
				AddUserWindow.getDefaultInsatnce().showWindow(currentSelectedUser.getUserID());
			// calls the communicator to add the user and updates the local data accordingly
			Utilities.getDefaultInstance().setCurrentUserData(ClientSideCommunicator.getDefaultInstance().getUserData(Utilities.getDefaultInstance().getCurrentUser()));
			// updates group data
			updateGroupData ();
		}
		...
		// when about the applicaiton option is selected
		if (AE_action.getSource() == MI_about)
		{
			// nneds to be changed to singleton class
			new AboutApp (this);
		}
	...
	private DefaultMutableTreeNode populateGroups ()
	{
		...
		rootUser = new UserSettings(TF_userID.getText(),TF_userID.getText(), true, UserStatus.UNKNOWN);
		...
		for (int i=0; i < userGroups.size(); i++)
		{
			...
			DefaultMutableTreeNode group = new DefaultMutableTreeNode (new UserSettings(groupName, groupName, true, UserStatus.UNKNOWN));
			// iterates all the users in the group and adds to the tree parent group node
			for (int j=0; j <us.length; j++)
			{
				DefaultMutableTreeNode users = new DefaultMutableTreeNode (us[j]);
				group.add(users);
				//System.out.println(us[j].getUserID() + us[j].getUserOfflineStatusMsg());
			}
			root.add(group);
		}
		
		// all group data is added in the last
		DefaultMutableTreeNode all = new DefaultMutableTreeNode (new UserSettings("all", "All Users", true, UserStatus.UNKNOWN));
		...
		while (iter.hasNext())
		{
			...
			//System.out.println("Online Status : "+ userID + allUsers.get(userID).getUserOnlineStatusMsg());
			//System.out.println("offline Status : "+ userID + allUsers.get(userID).getUserOfflineStatusMsg());
		}
		root.add(all);
		return root;
	}
	...
	{
		DefaultMutableTreeNode node = (DefaultMutableTreeNode)TR_userGroups.getLastSelectedPathComponent();
		if (node != null)
		{
	...
	public void mouseClicked(MouseEvent e) {
		...
		{
			DefaultMutableTreeNode node = (DefaultMutableTreeNode)TR_userGroups.getSelectionPath().getLastPathComponent();
			UserSettings us = (UserSettings)node.getUserObject();
			if (!us.isGroup())
			{
				String parent = ((UserSettings)((DefaultMutableTreeNode)node.getParent()).getUserObject()).getUserID();
				// checks whether a window is already opened and if it shows that window and if not adds a new window and shows that
				if (Utilities.getDefaultInstance().getOpenedWindow(us.getUserID()) == null)
				{
					Utilities.getDefaultInstance().addOpenedWindow(us.getUserID(), new ChatWindow(us.getUserID(), parent ));
				}
				Utilities.getDefaultInstance().getOpenedWindow(us.getUserID()).setVisible(true);
			}
		}
	}
	...
	public void windowClosing (WindowEvent WE_close)
	{
		...
		if (JOptionPane.showConfirmDialog(null, "Closing this window will signout you from the messenger", "Warning", JOptionPane.YES_OPTION) == JOptionPane.NO_OPTION)
			return;
		
	
		actionPerformed(new ActionEvent (MI_signout, ActionEvent.ACTION_PERFORMED, "SignOut"));
		//ClientSideCommunicator.getDefaultInstance().signOutUser(Utilities.getDefaultInstance().getCurrentUser());
		dispose () ;
		System.exit(0) ;
	}
	...
	private void cleanUpUserData ()
	{
		...
		if (users != null)
		{
			...
			{
				ChatWindow cw = Utilities.getDefaultInstance().getOpenedWindow(users[i]);
				cw.dispose();
				cw = null;
	...
	private void writeArchiveData ()
	{
		...
		for (int i=0; i < users.length; i++)
		{
			...
			String dayStr = Utilities.getDefaultInstance().appendLeadingZeros(day);
			String monthStr = Utilities.getDefaultInstance().appendLeadingZeros(month);
			File dir = new File ("archiveData/" + myUserID + "$"+ users[i]);
			if (!dir.exists())
			{
				dir.mkdirs();
			}
			File f = new File ("archiveData/" + myUserID + "$"+ users[i] +"/"+ year + monthStr + dayStr +".arch");
			...
			if (f.exists())
			{
				...
				{
					// reads old messages
					oldMessages = Utilities.getDefaultInstance().getArchivedMessages(f.getAbsolutePath());		
				}
				catch (Exception e)
				{
					e.printStackTrace();
			...
			Utilities.getDefaultInstance().writeArcieveData (f.getAbsolutePath(), oldMessages);
		}
	}
	...
	public void keyReleased(KeyEvent e) 
	{
		if (e.getSource() == TF_onLineStatus)
		{
			...
			{
				onlineMsg = TF_onLineStatus.getText() == null ? "" : TF_onLineStatus.getText();
				M_view.doClick();
				rootUser.setUserOnlineStatusMsg(onlineMsg);
				TR_userGroups.updateUI();
				ClientSideCommunicator.getDefaultInstance().setUserStatusMessage (Utilities.getDefaultInstance().getCurrentUser(),onlineMsg.replaceAll("\n", ""), true);
			}
			return;
		}
		...
		if (e.getSource() == TF_offlineStatus)
		{
			if (e.getKeyCode() == KeyEvent.VK_ENTER)
			{
				offlineMsg = TF_offlineStatus.getText() == null ? "" : TF_offlineStatus.getText();
				M_view.doClick();
				ClientSideCommunicator.getDefaultInstance().setUserStatusMessage (Utilities.getDefaultInstance().getCurrentUser(),offlineMsg.replaceAll("\n",""), false);
			}
		}
	}
