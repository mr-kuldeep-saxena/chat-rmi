 ...
 *
 */
public class OffLineMsgWindow extends JDialog implements ActionListener, ListSelectionListener, WindowListener {
	...
	/**
	 * adds offline message to window
	...
	{
		tableModel = new MessageTableModel (new Object []{"From", "Time", "Msg"}, 0);
		...
		TP_messages.setBorder(new CompoundBorder(new BevelBorder(BevelBorder.LOWERED), new EmptyBorder(3,3,3,3)));
		P_south.add (B_close);
		P_south.add (B_reply);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if (e.getSource() == B_close)
		{
			...
			ClientSideCommunicator.getDefaultInstance().deleteOfflineMessages (Utilities.getDefaultInstance().getCurrentUser());
			messages = null;
			setVisible(false);
			return;
		}
		...
		if (e.getSource() == B_reply)
		{
			...
			ChatWindow cw = Utilities.getDefaultInstance().getOpenedWindow(selectedMessage.getFromUser());
			if (cw == null)
			{
				cw = new ChatWindow(selectedMessage.getFromUser(), null);
				Utilities.getDefaultInstance().addOpenedWindow(selectedMessage.getFromUser(), cw);
			}
	...
	public void valueChanged(ListSelectionEvent e) 
	{
	     if (e.getValueIsAdjusting()) return;
	        ...
	        if (! lsm.isSelectionEmpty()) 
	        {
	        	   ...
	        	   Utilities.getDefaultInstance().showMessage(msg, this, TP_messages, displayBoxDoc, displayBoxStyle, false);
	        	   selectedMessage = msg;
	        }
	}
	 ...
	 * creates the false close button event
	 * and makes window hidden
	 */
	...
	/**
	 * window de iconified event handler
	 * does nothing here
	 */
