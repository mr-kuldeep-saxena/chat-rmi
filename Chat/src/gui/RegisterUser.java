...
/**
 * This class provides a GUI for client to resigster himself/herself to the system
...
public class RegisterUser extends JDialog implements ActionListener 
{
	 ...
	 */
	private JLabel L_userID, L_userName, L_userPassword, L_confirmPassword, L_userDOB, L_userAddress, L_userMobNo;
	private JTextField TF_userID, TF_userName, TF_userDOB, TF_userAddress, TF_userMobNo;
	...
	private void initComponents ()
	{
		...
		B_calendarLink = new JButton (new ImageIcon("resources/images/calendar.gif"));
		...
		addWindowListener(new WindowAdapter()
		{
			...
			{
				actionPerformed(new ActionEvent(B_close, ActionEvent.ACTION_PERFORMED,"Close"));
			}
		});
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if (e.getSource() == B_calendarLink)
		{
			...
			CalendarControl c;
			pop.add(c=new CalendarControl(TF_userDOB,Calendar.getInstance().get(Calendar.DAY_OF_MONTH), Calendar.getInstance().get(Calendar.MONTH),Calendar.getInstance().get(Calendar.YEAR)));
		...
		{
			if ( pop != null) pop.setForseVisible(false);
			dispose ();
			return;
		}
		...
		if (e.getSource() == B_register)
		{
			...
			if (mode.equalsIgnoreCase("Edit"))
			{
				...
				//ud = ClientSideCommunicator.getDefaultInstance().validateUser(ud.getUserID(), ud.getUserPassword(), (MessageReceiver)MessageReceiverImpl.getDefaultInstance());
	...
	{
		return new Point(B_calendarLink.getLocationOnScreen().x + 50,B_calendarLink.getLocationOnScreen().y + 15);
	}
	...
	{
		if (TF_userID.getText() == null || TF_userID.getText().trim().length() < 1)
		{
			JOptionPane.showMessageDialog(this, "Please enter valid user id");
			return false;
		}
		if (TF_userName.getText() == null || TF_userName.getText().trim().length() < 1)
		{
			JOptionPane.showMessageDialog(this, "Please enter your name");
			return false;
		}

		if (PF_userPassword.getPassword() == null || new String(PF_userPassword.getPassword()).trim().length() < 1)
		{
			JOptionPane.showMessageDialog(this, "Password is required");
			return false;
		}
		
		if (PF_confirmPassword.getPassword() == null || ! (new String(PF_confirmPassword.getPassword()).equals(new String(PF_userPassword.getPassword()))))
		{
			JOptionPane.showMessageDialog(this, "Password and confirm password did not match");
			return false;
		}
		return true;
	}
	 ...
	 * @param password user confirm pssword
	 */
