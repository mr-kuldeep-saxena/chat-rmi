...
/**
 * this class is the implemenation class of the ChatServer
 * it provides the real functionality to the Server, all
 * the messages which can be called from the remote client
 * are defined here. it makes use of DBOpeation to communicates with the DB
 * and ServerSideCommunicator to communicates with the client
 * To run the server stub for this class and MessageReceiver needs to be created
 * cmd: rmic -classpath . server.ChatServerImpl and rmic -classpath . client.MessageReceiverImpl 
 * this class has the main function to start the server  
 * @author Kuldeep Saxena
 *
 */
public class ChatServerImpl extends JFrame implements ChatServer, ActionListener
{
	/**
	 * @serial serial key
	 */
	 ...
	 * all user list
	 */
	//private HashMap<String,UserSettings> HM_users = new HashMap<String,UserSettings> ();
	...
	public ChatServerImpl ()
	{
		...
		try
		{
			...
			LocateRegistry.createRegistry(3090).rebind("ChatServer", (ChatServer)this);
		...
		catch(Error e)
		{
			...
			JOptionPane.showMessageDialog(null, "Please create stub classes for using this service");
			System.exit(-1);
		}
		catch (Exception e)
		{
			...
			JOptionPane.showMessageDialog(null, "Failed to register server with local system, Program will exit");
			System.exit(-1);
		}
	}
	...
	private void initComponents ()
	{
		...
		TA_message.setBorder(new CompoundBorder(new BevelBorder(BevelBorder.LOWERED), new EmptyBorder(5,5,5,5)));
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{
			// will be available in next version provides admin message to user from the server
		}
	 ...
	 * @param userID user to be signed in 
	 * @param pwd password
	 * @param client callback remote object
	 * @return UserData user's configuration stored at server
	 * @throws RemoteException in case of some error
	 */
	public UserData signInUser(String userID, String pwd, MessageReceiver client) throws RemoteException 
	{
	 ...
	 * @param userID for which data need to be fetched
	...
	public UserData getUserData(String userID) throws RemoteException 
	{
		...
		UserData ud = DBoperations.getDefaultInstance().loadUserSettings(userID);
		return ud;
	}
	 ...
	 * @param isNew whether a new user or editing the old data
	 * @return status of the operation
	 * @throws RemoteException in case of error
	 */
	public int signUpUser(UserData userData, boolean isNew) throws RemoteException 
	{
		if (isNew)
		{
			logger.writeLog("New user sign up request with user id " + userData.getUserID());
			TA_message.append("New user sign up request with user id " + userData.getUserID()+"\n");
		}
		else
		{
			logger.writeLog("Update account info request from user id " + userData.getUserID());
			TA_message.append("Update account info request from user id " + userData.getUserID() +"\n");
		}
		int status;
		if (isNew)
		{
			if (DBoperations.getDefaultInstance().isUserAlreadyExists (userData.getUserID()))
			{
				logger.writeLog("Sign up request failed, User with "+ userData.getUserID() +" already exists ");
				TA_message.append("Sign up request failed, User with "+ userData.getUserID() +" already exists \n");
		...
		if ( (status = DBoperations.getDefaultInstance().storeUserSettings(userData)) == OperationStatus.OPERATION_SUCESSFUL)
		{
			UserSettings us = new UserSettings (userData.getUserID(), userData.getUserName(), false, UserStatus.UNKNOWN);
			DBoperations.getDefaultInstance().getNewUsers().add(userData.getUserID());
			ServerSideCommunicator.getDefaultInstance().notifyNewUserStatus();
			/*HM_users*/DBoperations.getDefaultInstance().getAllUsers().put(userData.getUserID(), us);
			if (isNew)
	...
	public HashMap<String,UserSettings> getAllUsers() throws RemoteException 
	{
		...
		Set s = /*HM_users*/DBoperations.getDefaultInstance().getAllUsers().keySet();
		...
		while (iter.hasNext())
		{
			...
			{
				UserSettings us = /*HM_users*/DBoperations.getDefaultInstance().getAllUsers().get(id);
				us.setUserStatus(UserStatus.USER_ONLINE);
				/*HM_users*/DBoperations.getDefaultInstance().getAllUsers().put(us.getUserID(), us);
			}
		}
	 ...
	 * @param fromUser current user
	 * @param userID user to be added
	 * @param groupName group name
	 * @param isNewGroup whether group is new or not
	 * @return status of the operation
	 * @throws RemoteException in case of error
	 */
	public int addUserToGroup (String fromUser, String userID, String groupName, boolean isNewGroup) throws RemoteException
	{
		logger.writeLog("Request to add user to the group received from user [" + fromUser + "] User [" + userID + "] group name [" + groupName + "] Is a new group [" + (isNewGroup == true ? "Yes": "No") + "]");
		TA_message.append("Request to add user to the group received from user [" + fromUser + "] User [" + userID + "] group name [" + groupName + "] Is a new group [" + (isNewGroup == true ? "Yes": "No") + "]\n");
		int status = DBoperations.getDefaultInstance().addUserToGroup (fromUser,userID,groupName,isNewGroup);
	 ...
	 * @param userID user sending sign out request
	 * @throws RemoteException in case of some error
	 */
	public void signOutUser(String userID) throws RemoteException 
	{
		...
		UserSettings us = /*HM_users*/DBoperations.getDefaultInstance().getAllUsers().get(userID);
	 ...
	 * @throws RemoteException in case of error
	 */
	public Vector<Message> getOffLineMessages (String user) throws RemoteException
	{
	...
	/**
	 * function called to validate the user, and if its validated it is appended to
	 ...
	 * @return user's data
	 */
	private UserData validateUser (String userID, String pwd, MessageReceiver client)
	{
		UserData ud = DBoperations.getDefaultInstance().loadUserSettings(userID);
		if (ud  == null)
		{
		...
		if (ud.getStatus() == OperationStatus.OPERATION_SUCESSFUL)
		{
			...
			// updating lists
			DBoperations.getDefaultInstance().getActiveUsers().put(userID, client);
			...
			TA_message.append("Calling status updater to update user status to other user about new user status status\n");
			// updating signin notifier
			ServerSideCommunicator.getDefaultInstance().notifySignInUserStatus();
		}
		return ud;
	}
	 ...
	 * @param toUser user to file being send
	 * @param fromUser user sending file
	 * @param fileName file name
	 */
	public int sendFileAcceptanceToUser (String toUser, String fromUser, String fileName) throws RemoteException
	{
	...
	public int sendUserStatusMessage(String user, String message, boolean online) throws RemoteException 
	{
		...
		if (online)
		{
			...
			UserSettings us = DBoperations.getDefaultInstance().getAllUsers().get(user);
		...
		if (online)
		{
			DBoperations.getDefaultInstance().getOnlineStatusList().put (user, message);
			ServerSideCommunicator.getDefaultInstance().notifyOnlineStatus();
		}
		else
		{
			DBoperations.getDefaultInstance().getOfflineStatusList().put (user,message);
