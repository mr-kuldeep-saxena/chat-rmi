...
public class PopupMessageHandler extends Thread 
{
	...
	private void initComponents ()
	{
		...
		TA_message.setFont(new Font("Times New Roman", Font.BOLD|Font.ITALIC,14));
		...
		try
		{
			while (true)
			{
				for (int i=0; i < messages.size(); i++)
				{
					...
					pop.setLocation(Utilities.getDefaultInstance().getTaskBarBounds().width- mainPanel.getPreferredSize().width,Utilities.getDefaultInstance().getTaskBarBounds().y-mainPanel.getPreferredSize().height);
					pop.setVisible(true);
					pop.updateUI();
					Thread.sleep(5000);
				}
